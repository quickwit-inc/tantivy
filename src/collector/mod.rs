...
impl<TCollector: Collector> Collector for Option<TCollector> {
    ...
    fn requires_scoring(&self) -> bool {
        self.as_ref().map(|inner| inner.requires_scoring()).unwrap_or(false)
    }
    ...
    fn merge_fruits(
        ...
        if let Some(inner) =  self.as_ref() {
            let inner_segment_fruits: Vec<_> = segment_fruits.into_iter().flat_map(|fruit_opt| fruit_opt.into_iter()).collect();
